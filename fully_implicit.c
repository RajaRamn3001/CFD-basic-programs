...
double **full_implicit(double **T,int m,int n,double B,double C,double A,double w,double e,int *iteration)
{FILE *fpr=fopen("timehistory_midpt.txt","w");
 ...
 {residue_sqr_sum=0;
  ...
  { for(int j=1;j<m-1;j++)
    {
     T_n[i][j]=(T[i][j]+C*(T_n[i][j+1]+T_n[i][j-1])+A*(T_n[i+1][j]+T_n[i-1][j]))/B;
...
{FILE *fp;
 ...
 int m=X/dx+1;
 int n=Y/dy+1;
 double B=1+2*diffusivity*dt/pow(dx,2)+2*diffusivity*dt/pow(dy,2),C=diffusivity*dt/pow(dx,2),A=diffusivity*dt/pow(dy,2);
 ...
 int iter=0;
 temp=full_implicit(temp,m,n,B,C,A,relaxation=1,accuracy,&iter);
 fp=fopen("output2b.txt","w");
 printf("%d",iter);
 for(int i=0;i<n;i++)
 { for(int j=0;j<m;j++)
   {
