...
double **full_explicit(double **T,int m,int n,double B,double C,double A,double w,double e,int *iteration)
{FILE *fpr=fopen("timehistory_midpt.txt","w");
 ...
 {residue_sqr_sum=0;
  ...
  { for(int j=1;j<m-1;j++)
    {
     T_n[i][j]=B*T[i][j]+C*(T[i][j+1]+T[i][j-1])+A*(T[i+1][j]+T[i-1][j]);
...
{FILE *fp;
 ...
 int iter=0;
 temp=full_explicit(temp,m,n,B,C,A,relaxation=1,accuracy,&iter);
 fp=fopen("output2a.txt","w");
 printf("%d",iter);
 for(int i=0;i<n;i++)
 { for(int j=0;j<m;j++)
   {
